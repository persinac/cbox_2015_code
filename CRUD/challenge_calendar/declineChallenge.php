...
if($beforeStartDate == 0) {
	$stmt = $mysqli->prepare("update challenge_table set a_d = 'd' where challenge_id = ? AND challengee_id = ?"); 
	...
	if($result = $stmt->execute()) {
		...
		$testDB->SetDeclinedChallenge("declined challenge: " . $challenge->GetTitle() . "");
		$testDB->SubmitToActivityLog($testDB->GetDeclinedChallenge(), 1);
