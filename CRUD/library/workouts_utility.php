...
class WorkoutsUtility {
    ...
    public $log_obj;
    var $classForLog = "workouts_utility";
	...
	function BuildWorkouts($u, $opt) {
		...
		if ($result = $this->mys->query($query)) {
            $description = "UserID: $u OPT: $opt Query: SUCCESS";
            $this->log_obj->InsertIntoLog($this->classForLog, $function, $description);
		   while ($row = $result->fetch_assoc()) {
            ...
            $description = "UserID: $u OPT: $opt Query: FAILURE";
            $this->log_obj->InsertIntoLog($this->classForLog, $function, $description);
        }
	...
	function BuildWarmups($u, $d) {
		...
		$query = "SELECT  DATE_FORMAT(date, '%Y-%m-%d') AS date, warmup 
				 ...
				 ORDER BY date";		
		if ($result = $this->mys->query($query)) {
            $description = "UserID: $u DATE: $d Query: SUCCESS";
            $this->log_obj->InsertIntoLog($this->classForLog, $function, $description);
		   while ($row = $result->fetch_assoc()) {
            ...
            $description = "UserID: $u DATE: $d Query: FAILURE";
            $this->log_obj->InsertIntoLog($this->classForLog, $function, $description);
        }
	}
